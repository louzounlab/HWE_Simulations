    ...
    for i, alpha in enumerate(alpha_values):
        ...
        for alleles_amount in alleles_nums:
            for population_amount in population_amounts:
                ...
                population_amount_calculated, alleles_probabilities, probabilities, observed, observed_cdf, _ \
                    = gibbs_sampling.prepare_experiment_data(alleles_count=alleles_amount,
                                                             population_amount=population_amount,
                                                             alpha_val=alpha)
                print('Starting algorithm')
                p_value, elapsed_time = gibbs_sampling.full_algorithm(alleles_count=alleles_amount,
...
if __name__ == '__main__':
    ...
    # alpha_values = np.array([0.0, 0.2])
    # alleles_nums = np.array([10, 20, 50, 100, 250, 500, 1000])
    # population_amounts = np.array([100000, 1000000, 10000000])
